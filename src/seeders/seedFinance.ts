...
export async function seedFinance() {
  ...
  const admin = await User.findOne({ where: { email: 'admin@traceofthetide.com' } });
  const author = await User.findOne({ where: { email: 'author@traceofthetide.com' } });
  const editor = await User.findOne({ where: { email: 'editor@traceofthetide.com' } });
  ...
  const flagsData = [
    {
      ...
      description: 'Multiple failed payments detected - 5 failed attempts in the last hour',
      user_id: editor?.id,
      amount: 500,
      severity: 'high',
      status: 'open',
    ...
    {
      flag_type: 'multiple_failed_payments',
      description: 'Multiple failed payments detected - 3 failed attempts in the last hour',
      user_id: editor?.id,
      amount: 500,
      severity: 'high',
      status: 'open',
